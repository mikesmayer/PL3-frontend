...
(function ($, win) {
    win.oLog = function () {
        ...
        var toConsole = function (method, msg, data) {
            if (!config["verbose"]) {
                return
            }
        ...
        var post = function (data, callback) {
            if (!config.enabled) {
                toConsole("error", "oLogging is disabled");
                return
            }
            if (!valid) {
                toConsole("error", "oLogging is not properly initialized", data);
                return
            }
            $.ajax({
                type: "POST",
                ...
                toConsole("error", arguments)
            });
            return true
        };
        var buildURL = function () {
            return config.apiBaseURL + API_URL + visitorId + ".json"
        };
        var init = function () {
            visitorId = win.oDesk.getVisitorId();
            if (!visitorId) {
                toConsole("error", "oLogging could not be initialized, because there is no visitor cookie set");
                config.enabled = false
            } else {
                valid = true;
                authToken = CryptoJS.MD5(visitorId + "ubtkb3FP2A");
                toConsole("log", "oLogging initialized successfully")
            }
        };
        $.fn.ologData = function () {
            var that = this;
            return {
                name: that.data("olog-name"),
                location: that.data("olog-location"),
                sublocation: that.data("olog-sublocation"),
                data: that.data("olog-data"),
                htmlEvent: that.data("olog-html-event"),
                mixpanelEvent: that.data("olog-mixpanel-event"),
                mixpanelData: that.data("olog-mixpanel-data"),
                isValid: function () {
                    return this.name && this.location && this.sublocation && this.data
                },
                hasMixpanel: function () {
                    return this.mixpanelEvent && this.mixpanelData
                }
            }
        };
        init();
        return {
            log: function (event, location, subLocation, data, callback) {
                var object = {
                    event: event,
                    location: location,
                ...
                object["data"] = [];
                if ($.isArray(data)) {
                    object["data"] = $.merge(object["data"], data)
                } else {
                    object["data"].push(data)
                }
                var dataAsJson = JSON.stringify(object);
                var postedData = {
                    data: dataAsJson
                };
                return post(postedData, callback)
            },
            log_link: function (linkSelector, location, subLocation, data, event) {
            ...
            link_clicked: function (event) {
                ...
                var linkHref = event.data.href;
                try {
                    this.log(event.data.loggedEvent, event.data.location, event.data.subLocation, event.data.data, function () {
                        this.goToUrl(linkHref)
                    })
                } catch (ex) {
                    toConsole("error", ex)
                }
                window.setTimeout(this.goToUrl, config.sendTimeout, linkHref);
                return false
            },
            log_pageview: function (location, subLocation, data) {
                var that = this;
                $(document).ready(function () {
                    that.log("impression", location, subLocation, data)
                })
            },
            ...
            localMode: function (fakeVisitorId) {
                config["local"] = true;
                valid = true;
            ...
            auto_log: function () {
                var handler = $.proxy(this.attach_auto_log, this);
                $(document).ready(handler);
                $(document).on("o-refreshpage", handler)
            },
            attach_auto_log: function () {
                var that = this;
                var autoTrackedElements = $("*[data-olog=1]");
                if (!autoTrackedElements || autoTrackedElements.length == 0) {
                    return
                }
                autoTrackedElements.each(function (index, item) {
                    var item = $(item);
                    var ologData = item.ologData();
                    if (!ologData.isValid()) {
                        return
                    }
                    if (ologData.htmlEvent) {
                        item.on(ologData.htmlEvent, function () {
                            that.log_oLogData(ologData)
                        })
                    } else if (ologData.name == "click") {
                        if (item.prop("tagName") == "A") {
                            if (ologData.hasMixpanel()) {
                                var href = item.attr("href");
                                item.on("click", function () {
                                    oDesk.runWithTimeout(function () {
                                        var mixPanelDFD = $.Deferred();
                                        var loggingDFD = $.Deferred();
                                        that.toMixpanel(ologData.mixpanelEvent, ologData.mixpanelData, function () {
                                            mixPanelDFD.resolve()
                                        });
                                        that.log(ologData.name, ologData.location, ologData.sublocation, ologData.data, function () {
                                            loggingDFD.resolve()
                                        });
                                        $.when(mixPanelDFD, loggingDFD).done(function () {
                                            that.goToUrl(href)
                                        })
                                    }, function () {
                                        that.goToUrl(href)
                                ...
                                that.log_link(item, ologData.location, ologData.sublocation, ologData.data)
                            }
                        } else {
                            item.on("click", function () {
                                that.log_oLogData(ologData)
            ...
            log_oLogData: function (ologData) {
                this.log(ologData.name, ologData.location, ologData.sublocation, ologData.data);
                if (ologData.hasMixpanel()) {
                    this.toMixpanel(ologData.mixpanelEvent, ologData.mixpanelData)
                }
            },
            toMixpanel: function (event, data, callback) {
                if (win.mixpanel && config.enabled) {
